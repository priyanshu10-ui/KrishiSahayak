...
function renderDashboard() {
  ...
  el.innerHTML = `
    <div class="mb-8">
      <h2 class="font-[Lexend] text-2xl font-medium text-[#191c1c] mb-1">${t.fieldOverview}</h2>
      <p class="text-[#42493e]"></p>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <section class="md:col-span-12 lg:col-span-8 bg-white rounded-xl p-6 border border-stone-100 shadow-sm overflow-hidden relative">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h3 class="font-[Lexend] text-xl font-medium text-green-900 flex items-center gap-2">
              <span class="material-symbols-outlined">cloud_sync</span> ${t.weatherForecast}
            </h3>
            <p class="text-stone-500 text-sm font-semibold">${t.location}</p>
          </div>
          <div class="flex items-center gap-4 bg-stone-50 px-4 py-2 rounded-full border border-stone-100">
            <div class="flex items-center gap-2">
              ...
              <span class="font-bold">${t.humidityValue} ${t.humidity}</span>
            </div>
            <div class="w-px h-4 bg-stone-300"></div>
            <div class="flex items-center gap-2">
              <span class="material-symbols-outlined text-orange-500">air</span>
              <span class="font-bold">${t.windValue} ${t.kmph}</span>
            </div>
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
          <div class="md:col-span-2 flex items-center gap-4 bg-green-50/50 p-4 rounded-2xl border border-green-100/50">
            ...
            <div>
              <span class="text-4xl font-bold text-stone-900">${t.temperature}</span>
              <p class="text-stone-600 font-medium">${t.mostlySunny}</p>
            </div>
          </div>
          <div class="md:col-span-3 flex justify-between items-center gap-2 overflow-x-auto pb-2">
            ${[`${t.tue},partly_cloudy_day,29,stone-500`,`${t.wed},rainy,24,blue-400`,`${t.thu},thunderstorm,22,blue-600`,`${t.fri},cloud,26,stone-500`].map(d => {  
              const [day,icon,temp,color] = d.split(',');
              return `<div class="flex flex-col items-center p-3 min-w-[70px] ${day===t.thu?'bg-stone-50 rounded-xl border border-stone-100':''}">
      ...
      <section class="md:col-span-12 lg:col-span-4 bg-white rounded-xl p-6 border border-stone-100 shadow-sm">
        <h3 class="font-[Lexend] text-xl font-medium text-green-900 mb-6 flex items-center gap-2">
          <span class="material-symbols-outlined">trending_up</span> ${t.topMarketRates}
        </h3>
        <div class="space-y-4">
          ${[
            {name:t.wheat,price:'₹2,125',change:'+2.4%',up:true,color:'amber'},
            {name:t.rice,price:'₹1,940',change:'-0.8%',up:false,color:'blue'},
            {name:t.corn,price:'₹1,850',change:'+1.2%',up:true,color:'yellow'}
          ].map(c => `
            <div class="flex justify-between items-center p-3 hover:bg-stone-50 rounded-xl transition-colors border border-transparent hover:border-stone-100 cursor-pointer">
              <div class="flex items-center gap-3">
                ...
                <div><p class="font-bold text-sm">${c.name}</p><p class="text-xs text-stone-500">${t.perQuintal}</p></div>
              </div>
        ...
        <button onclick="navigateTo('market')" class="w-full mt-6 py-2 text-stone-500 font-semibold text-sm hover:text-green-800 flex items-center justify-center gap-2 group">
          ${t.viewAllMarkets} <span class="material-symbols-outlined text-sm transition-transform group-hover:translate-x-1">arrow_forward</span>
        </button>
      </section>
      ...
      <section class="md:col-span-12">
        <div class="flex justify-between items-center mb-6">
          <h3 class="font-[Lexend] text-xl font-medium text-green-900 flex items-center gap-2">
            <span class="material-symbols-outlined">potted_plant</span> ${t.currentCropHealth}
          </h3>
          <button class="bg-[#2d5a27] text-white px-4 py-2 rounded-full text-xs font-bold flex items-center gap-2 shadow-lg shadow-green-900/20 active:scale-95 transition-transform">
            <span class="material-symbols-outlined text-sm">add</span> ${t.addField}
          </button>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ${[
            {name:t.winterWheat,field:t.fieldA1,date:'Oct 12, 2023',status:t.optimal,statusColor:'green',stage:t.tillering,progress:65,ph:'6.8',moisture:'42%',
            ...
            {name:t.yellowMaize,field:t.fieldB4,date:'Nov 05, 2023',status:t.attention,statusColor:'amber',stage:t.vegetative,progress:30,
            ...
            <div class="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden group">
              ...
              <div class="p-5">
                <div class="flex justify-between items-start mb-4">
                  <div><h4 class="font-bold text-lg">${crop.name}</h4><p class="text-xs text-stone-500">${t.sown}: ${crop.date}</p></div>
                ...
                <div class="space-y-4">
                  <div>
                    <div class="flex justify-between text-xs mb-1"><span class="text-stone-500">${t.growthStage}</span><span class="font-bold">${crop.stage}</span></div>
                  ...
                  ${crop.ph ? `<div class="grid grid-cols-2 gap-4">
                    <div class="bg-stone-50 p-2 rounded-lg text-center"><p class="text-[10px] text-stone-400 uppercase font-bold">${t.soilPH}</p><p class="text-sm font-bold text-stone-700">${crop.ph}</p></div>
                    <div class="bg-stone-50 p-2 rounded-lg text-center"><p class="text-[10px] text-stone-400 uppercase font-bold">${t.moisture}</p><p class="text-sm font-bold text-stone-700">${crop.moisture}</p></div>
                  </div>` : ''}
          ...
          <div onclick="navigateTo('crop-health')" class="bg-white rounded-2xl border border-stone-100 shadow-sm border-dashed flex flex-col items-center justify-center min-h-[300px] text-stone-400 hover:text-green-800 hover:bg-green-50/50 transition-all cursor-pointer">
            ...
            <p class="font-bold">${t.addNewCropField}</p>
            <p class="text-xs">${t.monitorHealth}</p>
          </div>
        </div>
      </section>
    </div>
